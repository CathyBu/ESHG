...
for cishu in range(1, 11):
    for preName in preNameList:
        ...
        for preNum in range(0, finalNum+1):   #0~10
              ...
              with tf.variable_scope('DownstreamApplication'):
                  ...
                  if FLAGS.visualize == True and preNum==finalNum:
                          ...
                          for k in range(len(result[1][0])):
                            ...
                            if i < evalNum:
                              if preClass == True:
                                print('配伍评估药组：', medicalList)
                                # allDice.append(diceEval.evalMedicalDice(medicalList, evalData))
                                allList5.append(meList5)
                                allList6.append(meList6)
                                allList7.append(meList7)
                                allList8.append(meList8)
                      ...
                      if FLAGS.visualize == True and preNum==finalNum:
                        ...
                        f.write('</body></html>')
                        f.close()
                        processAllList.processAll(allList5,preName,cishu,'5')
                        processAllList.processAll(allList6,preName,cishu,'6')
                        processAllList.processAll(allList7,preName,cishu,'7')
                        processAllList.processAll(allList8,preName,cishu,'8')
            ...
            sess.close()
        # processAllList.processAll_load_acc(allAccList, preName,cishu)
