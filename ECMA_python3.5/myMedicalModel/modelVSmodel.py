...
def plotMetrics(step_losses, epoch_lossacc, preName, preNum):
    pic_path = '../myMedicalModel/modelvsmodel-attention1/acc_loss_pic/'
...
for preName in preNameList:
        ...
        with tf.Session() as sess:
          ...
          with tf.variable_scope('DownstreamApplication'):
              ...
              if FLAGS.train == True:
                ...
                saver.save(sess, '../myMedicalModel/modelvsmodel-attention1/model/model_noLstmHH_r1_%s_epoches%s_num%s.ckpt'%(preName,FLAGS.num_epochs,preNum))
              else:
                saver = tf.train.Saver()
                saver.restore(sess, '../myMedicalModel/modelvsmodel-attention1/model/model_noLstmHH_r1_%s_epoches%s_num%s.ckpt'%(preName,FLAGS.num_epochs,preNum))
              ...
              if FLAGS.visualize == True and preNum < finalNum:
                  k_count = 0
                  f = open('../myMedicalModel/modelvsmodel-attention1/html/%s_visualizeTCM_%s_noLSTM_HWH_epoches%s_r1_num%s.html' % (
              ...
              ##########################################################最终结果显示
              if FLAGS.visualize == True and preNum==finalNum:
                  f = open('../myMedicalModel/modelvsmodel-attention1/html/final_%s_visualizeTCM_%s_noLSTM_HWH_epoches%s_r1_num%s.html'%(preName,preName,FLAGS.num_epochs,preNum), 'w')
    ...
    data_process.write_list_in_csv_a('../myMedicalModel/modelvsmodel-attention1/diceRecord.csv',diceAllList)