...
class SelfAttentive(object):
  ...
  #d_a=350
  #r=30
  def build_graph(self,usePreVector=False,vectors=None,n=20, d=50, u=32, d_a=128, r=1, reuse=False,lamda = 0.0002):
    # tf.set_random_seed(sj)
    with tf.variable_scope('SelfAttentive', reuse=reuse):
      ...
      self.W_s1 = tf.get_variable('W_s1', shape=[self.d, self.d],
          initializer=initializer,regularizer=regularizer)
      #hnew和h拼接
      # self.W_s2 = tf.get_variable('W_s2', shape=[self.d_a, self.d*2],
      #不拼接，Q和H相乘
      ...
      self.A = A = tf.matmul(Q,H)  # 张
      # self.A = A = H  # yaopan消融
     # self.A = A = tf.concat([tf.matmul(Q,H),H],-1)  # yaopan 拼接h与hnew
      ...
      # self.M = tf.matmul(B, H)  #张
      self.M = tf.matmul(B, A)   # yaopan h_new
