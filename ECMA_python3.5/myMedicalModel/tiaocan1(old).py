...
tf.app.flags.DEFINE_float('learn_rate', 1e-4, 'learn rate for training optimization')
...
for xuexilv in [0.001,0.0005,0.0001]:
    for d_a in [32,64,128]:
        for zz in [0.001,0.0001]:
            lr = xuexilv
                                  ...
                                  for k in range(len(result[1][0])):
                                      ...
                                      for j in range(word_pad_length):   # 药物数量
                                          ...
                                          if len(ww) <= j:
                                              ...
                                              w = ww[j]
                                              # if result[1][0][k][j] >= a:
                                              if result[1][0][k][j] >= float(1.0) / (60 - N_0):
