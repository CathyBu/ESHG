...
def plotMetrics(step_losses, epoch_lossacc, preName, preNum):
    pic_path = '../myMedicalModel/atentionVision/acc_loss_pic/'
...
for preName in preNameList:
        ...
        with tf.Session() as sess:
          ...
          with tf.variable_scope('DownstreamApplication'):
              ...
              if FLAGS.train == True:
                ...
                saver.save(sess, '../myMedicalModel/atentionVision/model/model_noLstmHH_r1_%s_epoches%s_num%s.ckpt'%(preName,FLAGS.num_epochs,preNum))
              else:
                saver = tf.train.Saver()
                saver.restore(sess, '../myMedicalModel/atentionVision/model/model_noLstmHH_r1_%s_epoches%s_num%s.ckpt'%(preName,FLAGS.num_epochs,preNum))
              ...
              if FLAGS.visualize == True and preNum < finalNum:
                  ...
                  for i in range(int(total / batch_size)):
                      ...
                      if labelList[k_count]==[]:
                          ...
                          Q_value[k_count]=result[2]
                      else:
                          labelList[k_count][0] += thisLabel[0]
                          labelList[k_count][1] += thisLabel[1]
                          # print('yy', len(Q_value[k_count]),len(Q_value[k_count][0]))
                          for ii,iitem in enumerate(result[2]):
                              # print('zz',Q_value[k_count])
                              for jj,jitem in enumerate(iitem):
                                Q_value[k_count][ii][jj]+=jitem
                      k_count+=1
              ...
              if FLAGS.visualize == True and preNum==finalNum:
                  ...
                  f = open('../myMedicalModel/atentionVision/html/final_%s_visualizeTCM_%s_noLSTM_HWH_epoches%s_r1_num%s.html'%(preName,preName,FLAGS.num_epochs,preNum), 'w')
    ...
    data_process.write_list_in_csv_a('../myMedicalModel/atentionVision/visionRecord.csv',diceAllList)
