    ...
    for preName in preNameList:
      ...
      # for sup in [ 0.003,0.005,0.008]:  #张
       ...
       # 手动修改名称 end
        ...
        # filename='../ECMA_Apriori_10_20epochs_0.01_l2_1/ECMA_%s_Apriori_1.csv'%(preName)     # yaopan修改：ECMA结果apriori
        # filename = '../ECMA_Apriori_10_20epochs_0.01_l2_1/ECMA_%s_Apriori_16_hnew.csv' % (preName)  # yaopan修改：ECMA结果apriori-hnew
        # filename = '../%sFile/Apriori_%s_data.csv'%(preName,preName)            # yaopan修改：直接apriori
        # filename='../ECMA_AP/ECMA_%s_Apriori_16.csv'%(preName)     # yaopan修改：ECMA结果apriori-60
        # filename = '../ECMA_AP/ECMA_%s_Apriori_16_hnew.csv' % (preName)  # yaopan修改：ECMA结果apriori-hnew-60
        # filename = '../ECMA_AP/ECMA_%s_Apriori_16_avg.csv' % (preName)  # yaopan修改：ECMA结果apriori-60-avg
        # filename = '../ECMA_AP/ECMA_%s_Apriori_16_hnew_avg.csv' % (preName)  # yaopan修改：ECMA结果apriori-hnew-60-avg
        # filename = '../ECMA_AP/ECMA_%s_Apriori_16_0.001_128_0.002-1.csv' % (preName)  #
        filename = '../ECMA-627-Hnew+H-unit80-top5-8/ECMA_%s_Apriori_0.001_128_0.0002-top7-6.csv' % (preName)  #


        # writecsvname = 'tj_result1/%s_0618.csv' % preName   # yaopan修改
        # writecsvname = 'tj_result_hnew/%s_0618.csv' % preName    # yaopan修改 hnew
        # writecsvname = 'tj_result/%s_0618.csv' % preName    # yaopan修改
        # writecsvname = 'tj_result_60/%s_0618.csv' % preName  # yaopan修改 60
        # writecsvname = 'tj_result_hnew_60/%s_0618.csv' % preName    # yaopan修改 hnew-60
        # writecsvname = 'tj_result_60_avg/%s_0618.csv' % preName  # yaopan修改 60
        # writecsvname = 'tj_result_hnew_60_avg/%s_0618.csv' % preName    # yaopan修改 hnew-60
        # writecsvname = 'tj_result_60_0.001_128_0.002/%s_0618-1.csv' % preName  #
        writecsvname = 'tj_result_627_Hnew+H_unit80_top5-8/top7-%s-6.csv' % preName  #


        writeCsv = '../aprioriEvaDice/0627_%s_diceEvaluate.csv' % preName
        ...
        items  = tj(inFile)
        wn = '627/unit80_top7_%s_apriori-6.csv' % preName
      ...
      all_r.append(['','','',''])
    import csv
    with open('top_result-627-Hnew+H-unit80-top7-6.csv','wb') as fw:
        writer = csv.writer(fw)
