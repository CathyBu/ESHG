    ...
    for preName in preNameList:
      ...
      # for sup in [ 0.003,0.005,0.008]:  #张
       ...
       # 手动修改名称 end
        ...
        import os
        all_path = os.listdir('../ECMA_QRJD')
        for filename in all_path:
            print(filename)
            file = '../ECMA_QRJD/'+filename
            writecsvname = 'tj_QRJD_60_avg/'+filename  # yaopan修改 60
           ...
           # part 2 start
            ...
            all_r.append(['','','',''])
    import csv
    with open('top_result60_avg_QRJD.csv','wb') as fw:
        writer = csv.writer(fw)
